...
user_path = f"{di_path}users.json"
users_list = fun.read_data(user_path)["data"]
...
while user != 'q':
    ...
    user = fun.choose()
    if user == '1':
        fun.create_user(user_path,users_list)
    elif user == '2':
        if logout:
            veredict = fun.log_in(users_list)
            if veredict:
                logout = False
                fun.menu2_functions(data)   
            else:
